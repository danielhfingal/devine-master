...
(function () {
  ...
  /** Derive status flags from catalogue entry / export / meta when available */
  function sdDeriveStatusFromContext(name) {
    ...
    // Try live catalogue / window catalogue if present
    try {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        if (sdNorm(fn) === norm || fn.indexOf(song) >= 0 || song.indexOf(fn) >= 0) {
          hit = e;
          break;
        }
      }
      if (hit) {
        ...
        if (hit.mark || hit.provenance) out.streaming = out.streaming || !!hit.safetyPass;
        if (hit.onSpotify === true || hit.spotify === true) out.spotify = true;
        // Multiple export formats sometimes listed
    ...
    // Heuristic: if override exists or matched lyrics → local
    if (sdLoadOverride(name) != null || sdState.source === "LYRICS_DB" || sdState.source === "local edit") {
      out.local = true;
    }
    return out;
  }
  ...
  function sdRenderStatusTicks() {
    ...
    STATUS_KEYS.forEach(function (item) {
      ...
      // Auto-managed keys stay enabled so user can override; lyricsLocked mirrors lock
      if (item.id === "lyricsLocked") {
        ...
        cb.addEventListener("change", function () {
          ...
          if (label && sdSongName(n)) {
            label.textContent = sdSongName(n) + (sdState.locked ? " \uD83D\uDD12" : "");
          }
        });
    ...
    if (!note) {
      ...
      note.addEventListener("change", function () {
        ...
        const n = typeof fileName !== "undefined" ? fileName : "";
        try {
          localStorage.setItem(SD_STATUS_PREFIX + "note_" + sdNorm(n), note.value);
        } catch (e) {}
      });
    }
    try {
  ...
  function sdApplyLockedUI() {
    ...
    // Keep status ticks in sync for lyrics locked
    const host = sd$("sdStatusTicks");
    if (host) {
  ...
  function sdUpdateLabel(name) {
    ...
    if (song) {
      ...
      label.title =
        sdState.source === "LYRICS_DB"
          ? "Matched from LYRICS_DB (clean view)"
          : sdState.source === "local edit"
          ? "Local edit"
          : "Template";
    } else {
  ...
  function sdApplyMatchedForName(name) {
    ...
    // Strip old textual status block from below so we use interactive ticks instead
    below = below.replace(
      /Status:\s*\n(?:\s*\[\s*[xX ]\s*\][^\n]*\n?)+/gi,
      ""
    );
    ...
    // Always show song name in label when available
    sdUpdateLabel(name);
    ...
    // Status ticks: derive + render (collapsed panel stays collapsed)
    sdState.status = sdDeriveStatusFromContext(name);
    sdRenderStatusTicks();

    // Ensure below wrap is present (panel still collapsed until user clicks)
    const wrap = sd$("sdBelowWrap");
    if (wrap) {
      wrap.style.display = "";
    }
    // Do not auto-open panel
  ...
  function sdDoSave() {
    ...
    if (sdState.editing) {
      ...
      if (bodyEl) {
        ...
        sdRenderText(bodyEl.innerText, true);
      }
    }
    const metaEl = sd$("sdMeta");
  ...
  function sdWire() {
    ...
    if (editBtn) editBtn.onclick = function () {
      ...
      if (sdState.editing) {
        ...
        if (body) {
          // Flatten any section HTML into plain text for reliable editing
          const text = body.innerText;
          body.innerHTML = "";
    ...
    // INFO checkbox (same pattern as main desk infoToggle)
    if (infoCb) {
      infoCb.addEventListener("change", function () {
        const metaEl = sd$("sdMeta");
        if (infoCb.checked) {
          if (metaEl) {
            metaEl.textContent = "Clean lyrics on top \u00b7 Status/notes below \u00b7 Export clean = body only";
          }
        } else {
    ...
    // Ensure wrap visible so toggle is clickable
    const wrap = sd$("sdBelowWrap");
